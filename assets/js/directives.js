...
angular.module('acs.directives', [])
    ...
    .directive('materialSample', function ($http) {
            return {
                restrict: 'A',
                priority: 1000,
                link: function (scope, element, attrs) {
                    ...
                    // scope.colorSection = function(){
                    // 	alert();
                    // }

                    function createMoreColorsTooltips(n) {
                        n.qtip({
                            content: {
                                text: function () {
                                    var t = '<div class="ttTitle">' + n.siblings(".colorSwatches").find("li").length + ' Colors Available<a href="javascript:void(0);" class="closeTooltip"><\/a><\/div>' + $("<div />").append(n.siblings(".colorSwatches").clone()).html();
                                    return n.siblings(".colorSwatches").find("li").length > 56 && (n.parents(".productTile").length > 0 ? t += '<a href="' + n.parents(".productTile").find(".productTitle").attr("href") + '#step1" class="ttMoreLink">View All<\/a>' : n.parents("#featuredProductBanner").length > 0 && (t += '<a href="' + n.parents("#featuredProductBanner").find(".featuredProductTitleLink").attr("href") + '#step1" class="ttMoreLink">View All<\/a>')), $(t)
                                }
                            },
                            position: {
                                at: 'center left',
                            ...
                            style: {
                                classes: "qtip-light qtip-shadow qt-colorsAvailable",
                                tip: {
                                    width: 15,
                                    height: 10,
                                    mimic: "center",
                    ...
                    $(function () {
                        ...
                        $("html").on("click", function () {
                            $(".moreColors").qtip("hide")
                        });

                        $("body").on("click", ".moreColors", function (n) {
                            return n.stopPropagation(), $(".moreColors").not($(this)).qtip("hide"), $(this).attr("data-hasqtip") || createMoreColorsTooltips($(this)), !1
                        });
                        ...
                        // 		e = $(this).data("colorid"),
                        // 		r = [],
                        // 		t = $(".productTile[data-prodid=" + $(this).data("prodid") + "]"),
                        // 		c = $(this).data('colorizedroom'),
                        // 		ifcode = $(this).data('ifcode'),
                        // 		catCode = $(this).data('cat_code'),
                        // 		item = $(this).data('item'),
                        // 		product_desc = $(this).data('product'),
                        // 		coll_desc = $(this).data('coll_desc'),
                        // 		prodid = $(this).data("prodid");

                        // 	$('.buy_' + ifcode).attr('href', 'customizing/' + item + '&product=' + product_desc + '&prod_id=' + prodid + '&if=' + ifcode + '&coll_desc=' + coll_desc + '&id=' + e + '');
                        // 	$('.customize_' + ifcode).attr('href', 'customizing/' + catCode + '/' + prodid + '/' + e + '');
                        // 	$('.buy_' + ifcode).attr('href', 'material?item=' + item + '&product=' + product_desc + '&prod_id=' + prodid + '&if=' + ifcode + '&coll_desc=' + coll_desc + '&id=' + e + '');
                        // 	$('#familyItemCode' + ifcode).text(item);
                        // 	$('#applyFamImg' + ifcode).attr('src', c);
